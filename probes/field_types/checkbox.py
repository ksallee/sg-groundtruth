...
if not _lib.writes_allowed():
    ...
    r = c.post("/entity/versions", json={"project": {"type": "Project", "id": SANDBOX},
                                         "code": f"zzprobe_checkbox_{int(time.time())}"})
    ...
    if vid:
        ...
        rows.append("\n=== filter: positive control on the row now ticked in the sandbox")
        SBX = ["project", "is", {"type": "Project", "id": SANDBOX}]
        sb_base, _ = search([SBX])
        for label, filt in [("is True", ["flagged", "is", True]),
                            ("is False", ["flagged", "is", False])]:
            n, _ = search([SBX, filt])
            rows.append(f"  sandbox flagged {label:<9} -> {n} of {sb_base}")

        rows.append("\n=== clear: does a checkbox survive being written on create?")
        r2 = c.post("/entity/versions", json={"project": {"type": "Project", "id": SANDBOX},
                                              "code": f"zzprobe_checkbox_null_{int(time.time())}",
                                              "flagged": None})
        rows.append(f"  POST with flagged=null -> {r2.status_code} "
                    f"{r2.json()['data']['attributes'].get('flagged', '<absent>') if r2.ok else json.dumps(r2.json().get('errors'))}")
        if r2.ok:
            c.request("DELETE", f"/entity/versions/{r2.json()['data']['id']}")

        d = c.request("DELETE", f"/entity/versions/{vid}")
        rows.append(f"\n  cleanup: DELETE the throwaway Version -> {d.status_code}")
    else:
