...
def ensure(suffix):
    """Reuse the throwaway Version across runs — a sandbox should not grow a row per invocation."""
    code = f"{CODE}_{suffix}"
    r = search([SB, ["code", "is", code]], ["code"])
    if r.ok and r.json()["data"]:
        return r.json()["data"][0]["id"]
    r = c.post("/entity/versions", json={"project": {"type": "Project", "id": SANDBOX}, "code": code})
    return r.json()["data"]["id"]


A, B, C = (ensure(s) for s in "abc")
rows.append(f"\n=== sandbox Versions {CODE}_a/_b/_c created or reused")
