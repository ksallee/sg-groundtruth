...
component and so breaks on the `{type, id}` braces the corpus is full of). `agent-ui-annotation` draws the
feedback toolbar on the dev server and reaches no build. It is a custom element, so the site takes on no
second framework to run it.
